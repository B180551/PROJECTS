...
# Load the model
with open('./finalRFmodel', 'rb') as file:
...
def classify():
    ...
    employment_type = str(request.form.get("EmploymentType"))
    ...
    EmpType={'Self-employed':3,'Full-Time':2,'Unemployed':1,'Part-Time':0}

    # Prepare the input data for prediction
    input_data = [
        [age, income, loan_amount, credit_score, months_employed, interest_rate, loan_term, EmpType[employment_type], has_mortgage, has_dependents, has_co_signer]
    ]
    ...
    result = "Loan Approved" if prediction[0] == 0 else "Loan Denied"
